...
variable "role_name" {
  description = "Override for the endpoint-exposer permissions IAM role name. Defaults to nullplatform_{cluster_name}_endpoint_exposer_role."
  type        = string
  default     = ""
}
...
variable "iam_create_role" {
  description = "Whether to create the permissions role and its policies. Set to false when AUTH_TYPE=aws-cognito, since that scheme makes no AWS API calls and needs no IAM role."
  type        = bool
  default     = true
}
